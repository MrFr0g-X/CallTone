...
Speaker-diarized transcription using:
  ...
  - SenseVoice      → what was said + emotion + sound events
...
# ── paths ──────────────────────────────────────────────────────────────────────
MODEL_DIR  = os.path.join(os.path.dirname(__file__), "../models/sensevoice/iic/SenseVoiceSmall")
MODEL_CODE = os.path.join(os.path.dirname(__file__), "SenseVoice/model.py")
...
def parse_sense_output(raw: str) -> tuple[str, str, str]:
    """Return (clean_text, dominant_emotion, dominant_event) from raw SenseVoice output."""
    # Emotion detection disabled - only transcribe text
    events = []
    for m in SEGMENT_RE.finditer(raw):
        _, emo, evt, _ = m.groups()
        if evt != "Speech":
            events.append(evt)
    dominant_emo = "EMO_UNKNOWN"  # Always return unknown emotion
    dominant_evt = Counter(events).most_common(1)[0][0]  if events  else "Speech"
    clean = strip_emojis(rich_transcription_postprocess(raw))
    return clean, dominant_emo, dominant_evt
...
def transcribe_chunk(model, waveform: torch.Tensor, sample_rate: int) -> tuple[str, str, str]:
    ...
    # write to a temp wav so funasr can read it
...
def main():
    ...
    local_model_path = os.environ.get("PYANNOTE_MODEL_PATH")
    ...
    if local_model_path and os.path.exists(local_model_path):
        ...
        try:
            ...
            if not hf_token:
                ...
                print("  Option 1 - Use local model (RECOMMENDED):")
                print(f"    Set: export PYANNOTE_MODEL_PATH={Path(__file__).parent.parent}/pyannote_models/speaker-diarization-3.1")
                print("  Option 2 - Use HuggingFace:")
            ...
            print("  Set PYANNOTE_MODEL_PATH to use local models:")
            print(f"    export PYANNOTE_MODEL_PATH={Path(__file__).parent.parent}/pyannote_models/speaker-diarization-3.1")
            sys.exit(1)
    ...
    print("Step 2/3  Transcribing each speaker turn (SenseVoice)...")
