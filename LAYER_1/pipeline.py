...
def enhance_audio_stage(input_path, output_path, device="cuda" if torch.cuda.is_available() else "cpu"):
    ...
    enhanced_wav, new_sr = enhance(denoised_wav, new_sr, device, nfe=64, solver="midpoint", lambd=0.1, tau=0.5)
...
def transcribe_diarize_stage(audio_path, num_speakers=None):
    ...
    # Build command
    script_path = Path(__file__).parent / "layer_1" / "transcribe_diarize.py"
