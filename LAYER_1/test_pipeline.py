...
def process_audio(input_path, option, device="cuda" if torch.cuda.is_available() else "cpu"):
    ...
    if option == 2:
        # Only denoise
        print("\nDenoising audio...")
        processed_wav, new_sr = denoise(dwav, sr, device)
        ...
        processed_wav, new_sr = enhance(dwav, sr, device, nfe=64, solver="midpoint", lambd=0.5, tau=0.5)
        ...
        processed_wav, new_sr = enhance(denoised_wav, new_sr, device, nfe=64, solver="midpoint", lambd=0.1, tau=0.5)
